...
class STGCN4LSTM(nn.Module):
    def __init__(self, configs):
        ...
        self.conv4 = GCNConv(self.hidden_dim_1, self.hidden_dim_1)
        # LSTM Layer
        self.lstm = nn.LSTM(
...
# Define the ST-GAT with LSTM model
class STGATLSTM(nn.Module):
    def __init__(self, configs):
        super(STGATLSTM, self).__init__()
    ...
    def forward(self, data):
        ...
        x = x.view(-1, self.num_features)  # (B*WI*N, F)
        # Apply spatial graph convolutions
        x = F.relu(self.gat1(x, edge_index))  # (B*WI*N, H1)
